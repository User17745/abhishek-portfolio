...
test.describe("Chat Page", () => {
  ...
  test("should show send button enabled when input has text", async ({ page }) => {
    const input = page.getByPlaceholder(/Paste a job description/i);
...
test.describe("API Endpoint", () => {
  test("should return 400 when jdText is missing", async ({ request }) => {
  ...
  test("should return 500 when API key is missing", async ({ request }) => {
...
test.describe("Navigation", () => {
  test("should navigate to home from chat", async ({ page }) => {
    await page.goto("/chat");
