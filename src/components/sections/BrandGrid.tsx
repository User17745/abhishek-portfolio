...
export default function BrandGrid({ brands }: BrandGridProps) {
    ...
    return (
        <div className="flex flex-col items-center gap-10 w-full animate-in fade-in duration-700">
            <div className={cn(
                "grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-x-8 gap-y-12 items-start justify-items-center w-full transition-all duration-700 ease-in-out overflow-hidden",
                isExpanded ? "max-h-[4000px]" : "max-h-[320px] sm:max-h-[300px]"
            )}>
                {brands.map((brand, index) => {
                    ...
                    const visibilityClasses = cn(
                        ...
                        !isExpanded && index >= 12 ? "md:max-lg:hidden" : "",
                        !isExpanded && index >= 16 ? "lg:hidden" : ""
                    );
