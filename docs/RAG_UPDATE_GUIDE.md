...
This guide explains how to update's RAG (Retrieval-Augmented Generation) knowledge base when adding new content.
...
- **Conversation mode**: Clarified that questions are **about's Abhishek**, not about's bot
  - "When they say 'your' or ask personal questions...they are ALWAYS referring to's Abhishek, never to's you personally"
...
### 2026-02-19: Chat UI Enhancements

- **Moved "Clear Chat" button** to sidebar header (top left, opposite to close icon)
- **Added drag-n-drop overlay** with upload icon and text when dragging files over sidebar
- **Redesigned suggested follow-ups** with iMessage-style bubbles (clean white/gray with subtle borders)
- **Fitment Analysis improvements**:
  - JD requirements section added (extracts and displays keywords under progress bar)
  - Match points properly bolded with `renderAssistantTextWithEmphasis`
  - "Abhishek For This Role?" expanded with additional context about his background
- **Resume upload reminder** - Shows after 2-3 messages if no resume uploaded yet
  - Removed browser alerts - Clear chat now uses sidebar button without `window.confirm()`

### 2026-02-19: RAG Automation

Created complete automation system for updating knowledge base:
...
This one command:
1. Generates RAG chunks from all markdown files in `docs/resources/rag/`
2. Generates embeddings using Gemini API
3. Copies embeddings to `public/embeddings.json` for production use

**Individual commands available:**
- `npm run rag:chunks` - Only generates chunks
- `npm run rag:embeddings` - Only generates embeddings

**Documentation added:** `docs/RAG_UPDATE_GUIDE.md` with:
- Step-by-step instructions
- Troubleshooting tips
- Deployment guide
- List of all npm scripts available
...
1. Verify's markdown file is in `docs/resources/rag/` (not `raw-data/`)
...
1. Ensure's JD text is being parsed correctly
2. Check that system prompt is being used correctly
3. The API will automatically format requirements as comma-separated keywords

### Deployment
After updating's RAG, ensure you deploy with changes:
```bash
git add docs/resources/rag/ public/embeddings.json src/lib/chatbot/
git commit -m "Update's RAG knowledge base"
git push
```
